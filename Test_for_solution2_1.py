...
def generate_best_solution(settings, qs, cons, bounds, model, 
    ...
    if  np.max(predictions)> current_q_predictions:
        ...
        generate_best_solution(settings, qs, cons, bounds, model,
...
# settings.extend(best_sol) 
# settings = np.array(settings)
#新的对于np数组的操作
# settings = np.concatenate((settings, best_sol.reshape(1,-1)), axis=0) 

# qs = np.concatenate((qs,qs.reshape(1,-1)), axis=0)

# settings = np.concatenate((settings, best_sol.reshape(1,-1)), axis=0) 

# qs = np.concatenate((qs,q_list.reshape(1,-1)), axis=0)
