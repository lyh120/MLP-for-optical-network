...
def train_gan(real_configs, real_q, steps=200, noise_dim=16, device='cpu'):
    data_dim = real_configs.shape[1] + real_q.shape[1]
    real_data = np.hstack([real_configs, real_q])
    ...
    for step in range(steps):
        ...
        real_sample_idx = np.random.choice(len(real_data), batch_size)
        real_batch = real_data[real_sample_idx]
...
# -------- 关键：adaptive_online_finetuning集成GAN增广 --------
def adaptive_online_finetuning(model=None, 
    ...
    # GAN增广
    gan = train_gan(raw_configs, raw_q, steps=200, noise_dim=16, device=device)
    num_gen = 10
    z = torch.randn(num_gen, 16).to(device)
    gen_data = gan(z).detach().cpu().numpy()
    gen_configs, gen_q = gen_data[:,:12], gen_data[:,12:]
    # 合成数据和真实数据合并
    real_configs = np.vstack([raw_configs, gen_configs])
    real_q = np.vstack([raw_q, gen_q])
