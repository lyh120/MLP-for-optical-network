...
def adaptive_online_finetuning(model=None, 
    ...
    # 使用所有数据进行增强
    aug_configs = sliding_window_augmentation(setting_list)
    aug_q = sliding_window_augmentation(q_list)

    real_configs = np.vstack([setting_list, aug_configs])
    real_q = np.vstack([q_list, aug_q])
