...
class EnhancedLipschitzNN(nn.Module):
    def __init__(self, input_dim=12, output_dim=6):
        ...
        # # 将MLP替换为KAN（修改部分）
        # self.predictor = KAN(
        #     width=[32, 128, output_dim],  # 输入维度32，隐藏层128，输出6
        #     grid=5,  # 控制B样条的分段数
        #     k=3      # B样条的阶数
        # )
        # 修改KAN初始化参数（减少计算量）
        self.predictor = KAN(
            width=[32, 64, output_dim],  # 原128→64
            grid=3,             # 原5→3
            k=3,                # 保持3阶
            device='cuda' if torch.cuda.is_available() else 'cpu'  # 强制设备转移
        )
    ...
    def forward(self, x):
        ...
        return self.predictor(features)  # KAN的前向传播
...
# 2. 高精度离线训练（修改数据加载路径）
# ----------------------
def enhanced_offline_training(save_path="enhanced_kan_model.pth"):
    # 数据加载与预处理
    ...
    # 模型初始化（关键修改）
    model = EnhancedLipschitzNN().to(device)
    # # 模型初始化
    ...
    # 优化配置（调整学习率）
    optimizer = optim.AdamW(model.parameters(), lr=5e-4, weight_decay=1e-4)  # 降低学习率
...
# 3. 自适应在线微调（修改冻结策略）
# ----------------------
def adaptive_online_finetuning(model_path="enhanced_kan_model.pth", 
                              num_samples=5,   
                              epochs=50,       
                              lr=1e-4,        
                              lip_lambda=0.2): 
    ...
    start_idx = random.randint(1, 50)
    raw_configs = setting_list[start_idx:start_idx+num_samples*2]
    ...
    for name, param in model.named_parameters():
        if "feature_extractor" in name or "predictor.base_weight" in name:
    ...
    for epoch in range(epochs):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            patience_counter = 0
            torch.save(model.state_dict(), f"kan_fine_tuned_best.pth")
        else:
            patience_counter += 1
    ...
    def create_baseline_model():
        ...
        baseline_model.load_state_dict(torch.load(model_path, map_location='cpu')['model_state_dict'], strict=False)
        baseline_model.scaler_X = model.scaler_X
        baseline_model.scaler_y = model.scaler_y
        baseline_model.eval()
        return baseline_model

    test_indices = [15, 18, 22]
    ...
    with torch.no_grad():
        model.load_state_dict(torch.load("kan_fine_tuned_best.pth"))
    ...
    torch.save(checkpoint, f"kan_fine_tuned_best.pth")
